...
module.exports = class {
  ...
  //Login
  login (name, password) {
    if (this.data[name] === undefined) return { error: true, content: 'Account Not Found' }
    else if (password !== decrypt(Buffer.from(this.data.secret), Buffer.from(this.data.iv), this.data.accounts[name].password)) return { error: false, content: 'Wrong Password' }
    else return { error: false, session: '' }
  }
}
